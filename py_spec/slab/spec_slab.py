...
class SPECslab():
    ...
    def check_profile_Bfield(fname, func_by0, func_bz0):
        ...
        ax.plot(r, func_by0(r - np.pi), '--', picker=line_picker, label='B_y0')
        ax.plot(r, func_bz0(r-np.pi), '--', picker=line_picker, label='B_z0')
    ...
    def check_profile_curr(fname, func_jy0, func_jz0):
        ...
        ax.plot(r, J[2], '.-', picker=line_picker, label='SPEC J_phi', alpha=0.8)

        ax.plot(r, func_jy0(r - np.pi), '--', picker=line_picker, label='analytic J_y0')
        ax.plot(r, func_jz0(r - np.pi), '--', picker=line_picker, label='analytic J_z0')
    ...
    def check_profile_mu(fname, func_by0, func_bz0, func_jy0, func_jz0):
        ...
        mu_analytic = mu(x_array, func_by0, func_bz0, func_jy0, func_jz0)
    ...
    def check_profile_flux(fname, func_psi0, func_bz0):
        """
        ...
        x_array =  np.linspace(np.min(x_spec), np.max(x_spec), 200) - np.pi
        x_array = join_arrays([-np.pi], x_array)
        tflux_analytic = norm_torflux(x_array, func_bz0)
    ...
    def check_profile_iota(fname, func_by0, func_bz0):
        ...
        iota_analytic = iota_slab(r - np.pi, func_by0, func_bz0, rtor, rtor)
    ...
    def run_spec_slab(inpdict):
        ...
        mu_array = SPECslab.mu(x_array, inpdict.by0, inpdict.bz0, inpdict.jy0, inpdict.jz0)
        ...
        inputnml = py_spec.SPECNamelist(inpdict.fname_input)
        ...
        if(inputnml['physicslist']['Lconstraint'] == 0):
            check_psi(x_array, inpdict.psi0)
        ...
        # constrain the currents
            ...
            raise ValueError("Lconstraint has to be 0,1, or 3")

        inputnml.write_simple(inpdict.fname_input)

        SPECslab.run_spec(inpdict.fname_input, show_output=False)
...
if __name__ == "__main__":
    ...
    # psi_w = run_slab_loureiro()
    # run_slab_perturbed(psi_w)
    ...
    # perturb_eq('test.sp.h5', 0.05)
    # run_spec('pert_test.sp', show_output=True)
    # plot_kamsurf(fname='pert_test.sp.h5')

    # plotout = plot_kamsurf(fname='pert_test.sp.h5')
